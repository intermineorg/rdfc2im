...
def _write_status(ddir, per, ws, sources_cfg, out):
    ...
    L = [f"# rdfc2im - status ({today})", "",
         ...
         "| `fetch` (POST queries to RDF Portal) | **not run** - no network in the build sandbox; run `rdfc2im fetch` locally |",
         ...
         "| `humanmine-items` source | done: stock items-xml loader under its own type name + generated keys/additions; no Java |",
