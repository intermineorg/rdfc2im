...
from .items import table_root, _via_from
...
def gen_project(out_root: str, project_out: str, model: InterMineModel, type_name: str,
    ...
    # ...including the link fields those mappings go through. A column names the class and field it
    # WRITES; the collection or reference that connects it to its parent (Protein.keywords for an
    # OntologyTerm column) is named only by `via`, or inferred - so it is not in `writers`, and used
    # to be missed unless it happened to be core. When the stock source that declares it is dropped
    # from a reduced mine, nothing does, and the load dies "Collection not found in class: ...".
    to_carry = dict(writers)
    for src in source_dirs(out_root, only):
        for table, cols in load_columns(out_root, src).items():
            root_cls = table_root(cols)
            for c in cols:
                via = c.get("via") or (_via_from(model, root_cls, c["im_class"])
                                       if c["im_class"] and c["im_class"] != root_cls else "")
                if via:
                    to_carry.setdefault(via, [])
    carried = _carry_missing_definitions(model, to_carry, merged, classes_el)
