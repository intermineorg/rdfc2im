...
phase_start_databases() {
  local compose_files=(-f trial/docker-compose.yml)
  if [ "$BIND_ADDR" != "127.0.0.1" ]; then
    local override="trial/docker-compose.local.yml"
    write_file "$override" "$(cat <<YML
...
YML
  ...
  COMPOSE_FILES=("${compose_files[@]}")   # remembered for later phases
